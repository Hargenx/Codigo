// sudo apt install libncurses5-dev libncursesw5-dev # Ubuntu / Debian brew
// install ncurses # macOS (Homebrew)
#include <ncurses.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
  ...
  for (int y = 0; y < HEIGHT; y++) {
    for (int x = 0; x < WIDTH; x++) {
      ...
      if (c == 'P') {
        ...
        mapBuf[y][x] =
            (c == '#') ? WALL
                       : ((c == '.') ? PELLET : ((c == 'o') ? POWER : EMPTY));
      }
      if (mapBuf[y][x] == PELLET)
        g->pelletsRemaining++;
    }
    mapBuf[y][WIDTH] = '\0';
  }
...
static void draw_game(const Game *g) {
  ...
  mvprintw(
      HEIGHT, 0,
      "Score: %d   Lives: %d   Pellets: %d   (WASD/arrows to move, Q to quit)",
      g->score, g->lives, g->pelletsRemaining);

  if (g->gameOver) {
    const char *msg = g->win ? "YOU WIN! Press R to restart or Q to quit"
...
static int valid_move(int x, int y) { return !is_wall(x, y); }

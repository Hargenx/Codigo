...
import numpy as np
import pandas as pd
from ordem import Ordem  # Certifique-se de que a classe Ordem está definida em ordem.py
...
class Investidor:
    def __init__(
        ...
        piso_prob_negociar_val = self.params.get(
            "piso_prob_negociar", 0.1
        )  # Use 0.1 como default, ou seu padrão desejado
        self.prob_negociar = np.clip(
            piso_prob_negociar_val + 0.9 * ((1 - self.LF) ** 2), 0.1, 1.0
        )
        ####################################################################
    ...
    def calcular_preco_esperado(
        self, literacia_financeira: float, beta: float, dividendos: float
    ) -> float:
        ...
        def calcular_ema_curta_e_longa_otimizado(precos_historicos, LF):
            ...
            serie_precos = pd.Series(
                precos_historicos[-omega:]
            )  # pd.Series é eficiente para EMA
            ...
            ema_short = (
                serie_precos.ewm(alpha=alpha_short, adjust=False).mean().iloc[-1]
            )
        ...
        def calcular_sma_curta_e_longa_otimizado(precos_historicos, LF):
            ...
            janela_curta = max(
                2, int(omega / 4)
            )  # Janela curta, tipicamente 1/4 da longa
            ...
            if sma_long == 0 and sma_short == 0:
                ...
                sma_long == 0
            ):  # Se a SMA longa for zero, mas a curta não, use a curta como referência
                return sma_short, sma_short
            elif (
                sma_short == 0
            ):  # Se a SMA curta for zero, mas a longa não, use a longa como referência
        ...
        ema_short, ema_long = calcular_sma_curta_e_longa_otimizado(
            self.historico_precos, self.LF
        )
    ...
    def calcular_I_privada(
        self, n: int = 5, beta: int = 0, dividendos: float = 0.0
    ) -> float:
    ...
    def calcular_sentimento_risco_alocacao(
        self, mercado: "Mercado", vizinhos: list, parametros: dict
    ) -> None:
    ...
    def calcular_expectativa_inflacao(
        self,
        ...
        )  ###### -------------> Parâmetro peso_sentimento_inflacao

    def calcular_expectativa_premio(
        self, mercado: "Mercado", parametros: float
    ) -> None:
        self.expectativa_premio = mercado.banco_central.premio_risco * (
            1 - self.sentimento * parametros["peso_sentimento_expectativa"]
        )  ###### -------------> Parâmetro peso_sentimento_expectativa
    ...
    def calcular_estatisticas_retoricas(self) -> Optional[dict]:
        ...
        retornos = (
            self.historico_precos[1:] - self.historico_precos[:-1]
        ) / self.historico_precos[:-1]
    ...
    def criar_ordem(self, mercado: "Mercado", parametros: float) -> Optional["Ordem"]:
        ...
        preco_esperado = self.calcular_preco_esperado(
            ...
            beta=parametros["beta"],
            dividendos=mercado.fii.calcular_fluxo_aluguel()
            * 0.95
            / mercado.fii.num_cotas,
        )
